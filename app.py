...
def load_data():
    try:
        ...
        try:
            ...
            st.error("Data file not found. Please upload youtube_comments_topics.csv or youtube_comments_english.csv to your repo.")
            st.stop()
...
# ── RAG — only built when user visits Tab 4 ───────────────────────────────────
def build_rag_pipeline(df):
    ...
    class TFIDFEmbeddingFunction(EmbeddingFunction):
        def __init__(self, fit_docs, n_components=100):
    ...
    # Use a sample for cloud deployment to keep startup fast
    sample_size = min(5000, len(df))
    sample_df   = df.sample(sample_size, random_state=42).reset_index(drop=True)
    all_docs    = sample_df['text_clean'].tolist()
    ...
    # BM25 on same sample
    tokenized  = [str(t).lower().split() for t in all_docs]
    bm25_index = BM25Okapi(tokenized)

    groq_key    = st.secrets.get("GROQ_API_KEY", "")
    groq_client = Groq(api_key=groq_key) if groq_key else None

    return collection, bm25_index, all_docs, groq_client
...
def run_rag(query, collection, bm25_index, bm25_texts, groq_client, n=8):
    ...
    if not groq_client:
        return "Groq API key not configured in Streamlit secrets.", docs[:3], strategy
...
with tab1:
    ...
    with col_right:
        ...
        if 'keywords' in filtered.columns:
            all_kws = []
            for kw_val in filtered['keywords'].dropna():
                try:
                    kws = ast.literal_eval(kw_val) if isinstance(kw_val, str) else kw_val
                    if isinstance(kws, list):
                        all_kws.extend(kws)
                except:
                    all_kws.extend(str(kw_val).replace("'","").replace("[","").replace("]","").split(','))
            if all_kws:
                kw_freq = Counter([k.strip() for k in all_kws if k.strip()])
                ...
                st.info("No keywords found in filtered data.")
        else:
            st.info("Run Step 3 (keyword extraction) to enable word cloud.")

    st.subheader("Most Liked Comments")
    top_liked = filtered.nlargest(5, 'like_count')[['text_clean', 'sentiment_label', 'like_count']]
    for _, row in top_liked.iterrows():
        ...
        <div class="source-box">
            ...
            <span style="color:#888; font-size:0.8rem; margin-left:8px">👍 {int(row['like_count']):,} likes</span>
            <p style="margin-top:0.5rem; margin-bottom:0">{str(row['text_clean'])[:300]}</p>
...
with tab2:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(name='Positive', x=video_sent['video_id'], y=video_sent['Positive'], marker_color='#4ade80'))
    fig.add_trace(go.Bar(name='Negative', x=video_sent['video_id'], y=video_sent['Negative'], marker_color='#f87171'))
    fig.add_trace(go.Bar(name='Neutral',  x=video_sent['video_id'], y=video_sent['Neutral'],  marker_color='#facc15'))
    fig.update_layout(barmode='stack', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                      font_color='white', xaxis_title='Video ID', yaxis_title='Comments',
    ...
    for _, row in vid_df.sample(min(5, len(vid_df)), random_state=42).iterrows():
        ...
        <div class="source-box">
            <span class="{tag_class}">{row['sentiment_label']}</span>
            <p style="margin-top:0.4rem; margin-bottom:0">{str(row['text_clean'])[:250]}</p>
        </div>""", unsafe_allow_html=True)
...
with tab3:
    ...
    if 'topic_general' in df.columns and 'topic_general_label' in df.columns:
        topic_counts = (
            df[df['topic_general'] != -1]
            .groupby(['topic_general', 'topic_general_label'])
        ...
        fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                          font_color='white', height=380, xaxis_tickangle=-30, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)

        st.subheader("Explore a Topic")
        sel_label   = st.selectbox("Select topic", topic_counts['topic_general_label'].tolist())
        sel_id      = topic_counts[topic_counts['topic_general_label'] == sel_label]['topic_general'].values[0]
        topic_df    = df[df['topic_general'] == sel_id]

        c1, c2, c3 = st.columns(3)
        c1.metric("Comments", f"{len(topic_df):,}")
        ...
        for _, row in topic_df.sample(min(5, len(topic_df)), random_state=1).iterrows():
            ...
            <div class="source-box">
                <span class="{tag_class}">{row['sentiment_label']}</span>
                <p style="margin-top:0.4rem; margin-bottom:0">{str(row['text_clean'])[:280]}</p>
            </div>""", unsafe_allow_html=True)
    else:
        st.info("Topic columns not found. Run Step 4 (BERTopic) and use youtube_comments_topics.csv")
...
with tab4:
    ...
    st.markdown("Ask anything about fast fashion opinions — get answers grounded in real YouTube comments.")
    ...
    for i, (col, ex) in enumerate(zip(cols, example_queries)):
        if col.button(ex[:28] + "...", key=f"ex_{i}"):
            selected_example = ex
    ...
    if st.button("Search and Answer", type="primary") and query:
        with st.spinner("Building search index and generating answer... (first run takes ~30 seconds)"):
            try:
                if 'rag_collection' not in st.session_state:
                    collection, bm25_index, bm25_texts, groq_client = build_rag_pipeline(df)
                    st.session_state['rag_collection'] = collection
                    st.session_state['rag_bm25_index'] = bm25_index
                    st.session_state['rag_bm25_texts'] = bm25_texts
                    st.session_state['rag_groq']       = groq_client
                else:
                    collection  = st.session_state['rag_collection']
                    bm25_index  = st.session_state['rag_bm25_index']
                    bm25_texts  = st.session_state['rag_bm25_texts']
                    groq_client = st.session_state['rag_groq']
                ...
                if sources:
                    ...
                    for src in sources:
                        ...
                        <div class="source-box">
                            ...
                            <p style="margin-top:0.4rem; margin-bottom:0">{str(src.get('text',''))[:280]}</p>
                        </div>""", unsafe_allow_html=True)
            except Exception as e:
                st.error(f"Error: {str(e)}")
                st.info("Make sure GROQ_API_KEY is set in Streamlit Cloud secrets (App Settings → Secrets).")
...
with tab5:
    ...
    st.markdown("How has the sentiment and volume of fast fashion discussion changed?")

    time_df = df.dropna(subset=['month'])

    st.subheader("Comment Volume by Month")
    vol = time_df.groupby('month').size().reset_index(name='count').sort_values('month')
    ...
    fig2 = go.Figure()
    for sentiment, color in [('Positive','#4ade80'),('Negative','#f87171'),('Neutral','#facc15')]:
    ...
    yearly = (
        time_df.groupby(['year', 'sentiment_label'])
        .size().unstack(fill_value=0).reset_index()
    )
    yearly = yearly[yearly['year'] >= 2018]
    sent_cols = [c for c in ['Positive','Negative','Neutral'] if c in yearly.columns]
    fig3 = px.bar(yearly, x='year', y=sent_cols, barmode='group',
                  color_discrete_map={'Positive':'#4ade80','Negative':'#f87171','Neutral':'#facc15'})
